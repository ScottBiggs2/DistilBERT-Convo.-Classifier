#!/usr/bin/env python3
"""
Fine-tune DistilBERT for conversation intent classification with weighted loss
"""
...
class ModelConfig:
    ...
    use_weighted_loss: bool = True  # NEW: Enable/disable weighted loss
...
def train_model(config: ModelConfig, data_path: str, show_confusion_matrix: bool = False):
    ...
    # Initialize trainer - NEW: Choose weighted or standard trainer
    if config.use_weighted_loss:
        logger.info("Creating weighted loss trainer...")
        loss_config = LossWeightConfig(INTENT_LABELS)
        
        # Save the weight matrix for analysis
        weight_matrix_path = os.path.join(config.output_dir, "loss_weight_matrix.json")
        os.makedirs(config.output_dir, exist_ok=True)
        
        # Convert tensor to list for JSON serialization
        weight_matrix_list = loss_config.weight_matrix.tolist()
        with open(weight_matrix_path, 'w') as f:
            json.dump({
                'intent_labels': INTENT_LABELS,
                'weight_matrix': weight_matrix_list,
                'high_risk_categories': list(loss_config.high_risk_categories),
                'low_risk_categories': list(loss_config.low_risk_categories)
            }, f, indent=2)
        logger.info(f"Saved weight matrix to {weight_matrix_path}")
        
        # Create weighted trainer
        trainer = create_weighted_trainer(
            loss_config=loss_config,
            model=model,
            args=training_args,
            train_dataset=train_dataset,
            eval_dataset=val_dataset if not config.skip_evaluation else None,
            tokenizer=tokenizer,
            compute_metrics=compute_metrics if not config.skip_evaluation else None,
            callbacks=[EarlyStoppingCallback(early_stopping_patience=2)] if not config.skip_evaluation else None
        )
    ...
    if not config.skip_evaluation:
        ...
        print("="*60)
        if config.use_weighted_loss:
            print("*** WEIGHTED LOSS TRAINING ***")
        else:
            print("*** STANDARD LOSS TRAINING ***")
        ...
        with open(results_path, 'w') as f:
            json.dump({
                'weighted_loss_enabled': config.use_weighted_loss,
...
def main():
    ...
    config = ModelConfig(
        ...
        use_weighted_loss=not args.no_weighted_loss  # Default to weighted loss unless disabled
    )
    ...
    print(f"Weighted loss: {'ENABLED' if config.use_weighted_loss else 'DISABLED'}")
