...
def extend_position_embeddings(model, new_max_length=1024):
    ...
    # Copy old embeddings (positions 0-511) - these are the learned representations
    with torch.no_grad():
        # Copy weights
        new_embeddings.weight[:old_max_length].copy_(old_embeddings.weight)
        ...
        for i in range(old_max_length, new_max_length):
            ...
            new_embeddings.weight[i].copy_(
                (1 - blend_factor) * old_embeddings.weight[source_idx] + 
                blend_factor * old_embeddings.weight[next_idx]
            )
    ...
    logger.info(f"⚠️  Extended positions (512-{new_max_length-1}) initialized via interpolation")
...
class BERTDistillation:
    ...
    def __init__(self, config: DistillationConfig):
        ...
        if config.max_length > 512:
            ...
            self.model = BertForSequenceClassification.from_pretrained(
                ...
                ignore_mismatched_sizes=True  # Allow size mismatches during extension
            )
    ...
    def train(self, train_dataset: ConversationDataset, val_dataset: ConversationDataset,
        ...
        # CRITICAL: Move model to device BEFORE passing to trainer
        ...
        logger.info(f"   - config max_position_embeddings: {trainer.model.config.max_position_embeddings}")
        
        if actual_pos_emb_size != self.config.max_length:
            logger.error(f"❌ Position embedding size mismatch! Expected {self.config.max_length}, got {actual_pos_emb_size}")
            raise ValueError(f"Position embeddings not properly extended! Got {actual_pos_emb_size}, expected {self.config.max_length}")
        logger.info(f"✅ Position embeddings verified: {actual_pos_emb_size} positions")
